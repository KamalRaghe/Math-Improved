...
export default function Algebra({num1,num2,close}){  
    ...
    function Count(){
        ...
        if(step2 === true){
            setStep2(false)
            setStep3(true)
            setQ1(num2*num1 > 0 ? num1*num2:-1*(num2*num1))
            setQ2(num1)
    ...
    return (
        <div className="Help">
            ...
            {step1 && <div className="double center"> <button className="carry Green" onClick={() => {setAnswers(1);setSign('÷');setQ2(num1);setQ1(num1);setDone(false);setStep2(true);setStep1(false)}} >{num1}</button>𝑥 = {num2*num1 > 0 ? num1*num2:-1*(num2*num1)} </div>}
            {step2 && <div className="double center"><span className="center column">
                ...
                </span>{num1}</span>𝑥 = <span className="hide">.</span> <span className="center column">
                <span style={{borderBottom:'2px solid black'}} >
                <span className="hide">.</span>
