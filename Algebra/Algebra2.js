...
export default function Algebra1({num1,num2,close}){  
    ...
    return (
        <div className="Help">
            ...
            {step2 && <div className="double center" >𝑥 <span className="hide" >.</span><span className="across2" > = {num2+num1}</span> <span className="hide" >.</span> <span className="across" ><button  onClick={() => {setStep2(false);setStep3(true);setDone(false)}} className="carry Red" > + </button></span> <span className="hide" >.</span> <span className="across" >{num1}</span> </div>}

            {step3 && <div className="double center" >𝑥 = {num2+num1} <span className="hide" >.</span> - <span className="hide" >.</span> {num1} </div>}

            {step4 && <div className="double center Green " >𝑥 = {num2}</div>}
            ...
            {!done && !step4 && <div className='center wrap absolute StepAnswer'>
                   <Step value = {step4 ? "𝑥"+"="+ (answer+arr[0]) :answer+arr[0]}  answer={ step4 ? "𝑥"+"="+answer :answer} Count ={Count} done = {done} mistake={Nothing}/>
                   <Step value = {step4 ? '𝑥'+'='+ (answer+arr[2]) : answer+arr[2]}  answer={ step4 ? "𝑥"+"="+answer :answer} Count ={Count} done = {done} mistake={Nothing}/>
                   <Step value = {step4 ? '𝑥'+'='+ (answer+arr[3]) : answer+arr[3]}  answer={ step4 ? "𝑥"+"="+answer :answer} Count ={Count} done = {done} mistake={Nothing}/>
                   <button className="choice" style={{backgroundColor:'yellow',color:'black'}} onClick={()=>{setExtra(true);console.log(extra)}} >help</button>
                   <Step value = {step4 ? '𝑥'+'='+ (answer+arr[1]) : answer+arr[1]}  answer={ step4 ? "𝑥"+"="+answer :answer} Count ={Count} done = {done} mistake={Nothing}/>
