...
import TriH from "./Trihelp"


export default function Tri3({num1,num2,close}){  
    ...
    return (
        <div className="Help">
            ...
            {extra && sign === '÷' && <HelpDiv close={Extra} num1 ={Q1} num2 = {Q2}/>}
             {tri && <TriH num1={Q1} num2={Q2} close={()=>setTri(false)} ></TriH>}
            ...
            {before2 && <div className=" wrap double Green center" style={{padding:'20px'}} >{count1.map( num =>{
            return <div className="carry" style={{padding:'10px'}} onClick={()=>{setDone(false);setSign('+'); setAnswers(num+(num1*num2/num)); setBefore2(false);setQ2(num1*num2/num);setQ1(num);setStep1(true)}}>{num}:{num1*num2/num}</div>
          })}</div>}

            {!done && !before2 &&<div className=" double center Green absolute StepQuestion">{Q1} {sign} {Q2} = {extra && answer+'𝑥'}</div>}
               ...
               { done && !before1 && !before2 && !step4 &&<div className='center wrap StepAnswer' style={{width:"100%"}}>
                   { !step5 ? <button className="choice" onClick={()=>{setStep5(true);setTimeout(() => {
                        setStep5(false)
                   }, 1000)}} >+</button>: <button className="choice red" >+</button>}
                   <button className="choice" onClick={()=>{setStep2(false);setStep3(step3 ? false:true);setStep4(step3? true:false)}} >-</button>
               </div>}
                {done && !before1 && !before2 && !step4 && <div className = 'center' ><button className="choice help" style={{color:'black'}} onClick={()=>{setTri(true)}} >help</button></div>}
        </div>
    )
}