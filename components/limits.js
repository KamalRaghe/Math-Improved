        ...
        <div className="Help column" style={{zIndex:'50'}}>
            ...
            <div className='cancel'><button className='cancel-btn' onClick = {close}>X</button></div>
              ...
              {steps > 0 && <div style={{marginTop:"20px",width:"100%",fontSize:"25px",color:"black"}}>
               ...
               =    {steps > 4 && steps < 7 && <span style={{margin:"5px"}} > {c[0]*(x**e[0])}</span>}
                     {steps > 5 && steps < 7 && <span style={{margin:"5px"}} >+ {c[1]*(x**e[1])}</span>}
                     {steps == 7 && <span style={{margin:"5px"}} > {c[0]*(x**e[0])+c[1]*(x**e[1])}</span>}
                     {steps == 8 && <span style={{margin:"5px"}} > + {c[0]*(x**e[0])+c[1]*(x**e[1])+c[0]*(x**e[0])+c[1]*(x**e[1])}</span>}
                     {steps > 6 && steps < 8 && <span style={{margin:"5px"}} >+ {c[2]*(x**e[2])}</span>}
               </div>}
               <div className="box center" style={{width:"100%",height:"10%",alignItems:"center"}} >
                {extra && sign == 'x' && <TimesCalculator close={()=> {setExtra(false)}}></TimesCalculator>}
