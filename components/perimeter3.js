...
export default function Pm3({num1,num2,close}){
    const [done, setDone] = useState(false)
    const [extra, setExtra] = useState(false)
    const [Q1, setQ1] = useState(num1+num2)
    const [Q2, setQ2] = useState(2)
    const [sign ,setSign] = useState('x')
    const [answer, setAnswer] = useState((num1+num2)*2)
    ...
    function Count(){
        if(step1 === true){
            ...
            setQ2(num1)
            setSign('+')
            setAnswer(num1+num2+num1+num2+num1)
            setStep1(false)
        }else{
            close()
        }   
            ...
            {extra && sign === '÷' && <HelpDiv close={Extra} num1 ={Q1} num2 = {Q2}/>}
               ...
               {<div className="double relative" style={{top:'100px',left:'5px',color:'white',zIndex:'10'}} >P =</div>}
                {<div><div className="double" style={{borderBottom:"100px solid black",borderRight:'50px solid transparent',borderLeft:'50px solid transparent'}}></div>
                    ...
                    <div className="relative center Green" style={{top:"-112px",left:"-40px",fontSize:'20px'}} >{ num1+num2}</div>
                </div>}    
               </div>
