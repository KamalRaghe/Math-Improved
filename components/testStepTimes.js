...
function TestStepTimes({close, num1 , num2, mistake}){
...
function Count(){
    ...
    if(step1 === true){
     ...
     return(
        <div className="Help">
            <div className='cancel' style={{paddingBottom:'0px'}}><button className='cancel-btn' style={{paddingBottom:'0px'}} onClick = {close}>X</button></div>
            <div className='center absolute' style={{left:'155px', top: '60px'}}>
                <span className='hide'>00..</span>
                {carry && !stop && !step3 ? <button className='back2Step2 Green' onClick={click}>{sign}</button>:null}
                ...
                {(num1%10)*((num2-(num2%10))/10)>10 && step4===true && !stop1? <button className='Green back2Step2'  onClick={click2}>{sign5}</button>:null}
            </div>
            ...
            {step7 && (Math.floor(number3/10))%10+(Math.floor(number4/10))%10 >= 10 && !last? <span className='center' style={{width:'335px'}}><button className='back2Step2 Green' onClick={click3}>{sign4}</button></span>:null}
            {last ? <span className='center' style={{width:'335px'}}><button className='back2Step2 Green' onClick={click3}>{sign4}</button></span>:null}
            <div className='double center' style={{width: move2}} > 
                {step7 && (Math.floor(number3/10))%10+(Math.floor(number4/10))%10 >= 10 && last && <span className='Green lower-number' style={{paddingRight: "5px"}} >+</span>}
            ...
            {step6 ? <div className='center double Green' style={{width: '325px'}}><span className='hide'>.0000</span>{number3%10+number4%10}</div>: null}
            {step7 ? <div className='center double Green' style={{width: '325px'}}><span className='hide'>.000</span>{(number3%100+number4%100)%100}</div>: null}
            {step8 ? <div className='center double Green' style={{width: '325px'}}><span className='hide'>0.</span>{number3+number4}</div>: null}
            ...
            <div className='center wrap absolute StepAnswer'>
                <Step value = { num8 + num6 + num4 + answer[count]+arr[1]}  answer={ num8 + num6 + num4 + answer[count]} Count ={Count} done = {done} mistake = {mistake}/>
                <Step value = { num8 + num6 + num4 + answer[count]+arr[2]}  answer={ num8 + num6 + num4 + answer[count]} Count ={Count} done = {done} mistake = {mistake}/>
                <Step value = { num8 + num6 + num4 + answer[count]+arr[0]}  answer={ num8 + num6 + num4 + answer[count]} Count ={Count} done = {done} mistake = {mistake}/>
                <Step value = { num8 + num6 + num4 + answer[count]+arr[3]}  answer={ num8 + num6 + num4 + answer[count]} Count ={Count} done = {done} mistake = {mistake}/>
            </div>
        </div>
    )
}
export default TestStepTimes