...
export default function DoubleAdd({Count}){
    ...
    const [num6, setNum6] = useState(Math.ceil(Math.random()*2));
    const [num7, setNum7] = useState();
    const [num8, setNum8] = useState(Math.ceil(Math.random()*2))
    ...
    function Add(){
        setTimeout(() => {
            ...
            setNum6(Math.ceil(Math.random()*2))
            setNum8(Math.ceil(Math.random()*2))
            mix()
  ...
  const [score, setScore] =useState(0)
    const [count, setCount] =useState(0)
     ...
     useEffect(() =>{
         Add()
         setLoaded(true)
         const count = parseInt(window.localStorage.getItem(`${id} MMF`))
     ...
     useEffect(() =>{
         if(count > 0){
         window.localStorage.setItem(`${id} MMF`, count)
     }},[count])
    ...
    return(
        <div className="beige container column">
            <div className="Test sb"><div className="double" >
                ...
                <div className="font" >Multiplication Mixed Fraction: {loaded && count} </div>
            </div><Link href={`/${id}/${username}/MMFTest`}><button className="green test-btn">Test</button></Link></div>
            <div className="box">
            ...
            {help && <MMF num1={num4} num2={num5} close={close} num4={num2} num5={num1} whole1={num6} whole2={num8}  />}
