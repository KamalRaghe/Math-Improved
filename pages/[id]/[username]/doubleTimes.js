...
export default function DoubleAdd({Count}){
    ...
    useEffect(() =>{
     ...
     useEffect(() =>{
         setLoaded(true)
         const count = parseInt(window.localStorage.getItem(`${id} DoubleTimes`))
     ...
     useEffect(() =>{
         if(count > 0){
         window.localStorage.setItem(`${id} DoubleTimes`, count)
     }},[count])
    ...
    return(
        <div className="beige container column">
            ...
            </div><Link href={`/${id}/${username}/testDoubleTimes`}><button className="green test-btn">Test</button></Link></div>
