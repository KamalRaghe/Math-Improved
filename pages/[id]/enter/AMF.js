...
export default function DoubleAdd({Count}){
    ...
    const [count, setCount] =useState(0)
     ...
     useEffect(() =>{
         Add()
         setLoaded(true)
         const count = parseInt(window.localStorage.getItem(`${id} AMF`))
     ...
     useEffect(() =>{
         if(count > 0){
         window.localStorage.setItem(`${id} AMF`, count)
     }},[count])
    ...
    return(
        <div className="beige container column">
            <div className="Test sb"><div className="double" >
                ...
                <div className="font" >Addition Mixed Fraction: {loaded && count} </div>
            </div><Link href={`/${id}/${username}/AMFTest`}><button className="green test-btn">Test</button></Link></div>
            <div className="box  ">
