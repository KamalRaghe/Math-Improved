...
export default function DoubleAdd({Count}){
    ...
    const [count, setCount] =useState(0)
     ...
     useEffect(() =>{
         setLoaded(true)
         const count = parseInt(window.localStorage.getItem(`${id} Factor Trinomial`))
     ...
     useEffect(() =>{
         if(count > 0){
         window.localStorage.setItem(`${id} Factor Trinomial`, count)
     }},[count])
    ...
    return(
        <div className="beige container column">
            ...
            </div><Link href={`/${id}/${username}/TrinomialTest`}><button className="green test-btn">Test</button></Link></div>
            ...
            {help && num === 0 && <Tri num1 ={num1} num2={num2} close={close}/>}
            {help && num === 1 && <Tri1 num1 ={num1} num2={num2} close={close}/>}
            {help && num === 2 && <Tri2 num1 ={num1} num2={num2} close={close}/>}
