        ...
        <div className="beige container column">
           ...
           <div className="inTest">
            ...
            <div className="center" style={{width:"100%"}}>
                {loaded &&<div className="double tooBig" >𝑥<span style={{fontSize:'20px',position:'relative',padding:"2px", top:"-13px",paddingLeft:"2px",paddingRight:"10px"}}>2</span>
                    {sign1[num]} {(num ===2 || num === 1) ? num1-num2 : num1+num2}x {sign2[num]} {num1*num2} = 0
                </div>} 
            </div>
            <div className="box" style={{width:"340px"}}>
                {correct && <div className="Green double" >𝑥 = {(num === 0 || num === 2) ?-1*(num1+num3[2]) :num1+num3[2]} or 𝑥 = {(num === 0 || num === 1) ? -1*num2+num3[2]:num2+num3[2]}</div>}
                {wrong && <div className="Red double" >𝑥 = {(num === 0 || num === 2) ?-1*(num1+num3[2]) :num1+num3[2]} or 𝑥 = {(num === 0 || num === 1) ? -1*num2+num3[2]:num2+num3[2]}</div>}
            </div>
            { time - Date.now() < 0 && <Timeout again ={Again}/>}
            {mistake === 3 && <Mistake again={Again}></Mistake>}
            {count === 50 && <Pass time ={600000 -(time-Date.now())}/>}
            <div className="box column tooBig">
                <div className="row ">
